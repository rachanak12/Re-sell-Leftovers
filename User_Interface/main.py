...
# Load the model
price_model = load_model(r"User_Interface/model.h5")
...
if choice == "Create Account to Sell":
    ...
    with st.form("see_your_products"):
        ...
        if see_products:
            ...
            if seller:
                ...
                st.write(f"Total Revenue: Rs.{total_revenue:.2f}")
            else:
                st.warning("Username and password do not match. Please try again.")
    ...
    if not sales_data.empty:
        ...
        fig2 = px.line(sales_data, x="product_name", y="revenue", title="Revenue Distribution by Product", markers=True)
